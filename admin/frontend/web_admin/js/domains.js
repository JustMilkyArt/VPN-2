...
const TYPE_LABELS = {
  ...
  swagger:       { text: 'Swagger / API docs',  cls: 'bg-yellow-100 text-yellow-700' },
  client_site:   { text: 'Клиентский сайт',    cls: 'bg-purple-100 text-purple-700' },
  naiveproxy_eu: { text: 'NaiveProxy EU',       cls: 'bg-blue-100 text-blue-700' },
  naiveproxy_ru: { text: 'NaiveProxy RU',       cls: 'bg-red-100 text-red-700' },
  vpn:           { text: 'VPN (legacy)',         cls: 'bg-green-100 text-green-700' },
...
const SUBDOMAIN_TYPE_HINTS = {
  admin_panel:    '🖥 Создаст A-запись DNS, выпустит SSL и настроит Nginx с security headers. Блокирует прямой доступ по IP.',
  swagger:        '📄 Создаст A-запись DNS, выпустит SSL и откроет доступ к Swagger UI / API docs по этому поддомену.',
  client_site:    '🌐 Создаст A-запись DNS, выпустит SSL и настроит Nginx для клиентского сайта.',
  naiveproxy_eu:  '🇪🇺 Зарезервирует поддомен для NaiveProxy EU-сервера. SSL выпустит Caddy автоматически при запуске.',
  naiveproxy_ru:  '🇷🇺 Зарезервирует поддомен для NaiveProxy RU-сервера. SSL выпустит Caddy автоматически при запуске.',
...
function showCreateSubdomainModal(domainId) {
  ...
  // Заполняем суффикс домена
  const suffix = document.getElementById('subdomain-domain-suffix');
  if (suffix) suffix.textContent = `.${domain.name}`;
  ...
  document.getElementById('create-subdomain-btn').innerHTML = '<i class="fas fa-rocket text-xs"></i>Создать';

  // Показываем подсказку для дефолтного типа
  onSubdomainTypeChange();

  modal.classList.remove('hidden');
  document.getElementById('subdomain-name-input').focus();
}
...
// Обновляет подсказку при смене категории
function onSubdomainTypeChange() {
  const type = document.getElementById('subdomain-type-select')?.value;
  const hint = document.getElementById('subdomain-type-hint');
  if (hint && type) {
    hint.textContent = SUBDOMAIN_TYPE_HINTS[type] || '';
  }
}

// Живой предпросмотр полного имени поддомена
...
async function submitCreateSubdomain() {
  ...
  btn.disabled = false;
  btn.innerHTML = '<i class="fas fa-rocket text-xs"></i>Создать';
  ...
  // Запускаем поллинг статуса если нужно
