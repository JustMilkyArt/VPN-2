...
let _setupLastLogLen = 0;

const SETUP_STEP_MAP = {
  step1: 1,
  step2: 2,
  step3: 3,
  step4: 4,
...
function openServerSetupModal(serverId, serverName) {
  _setupServerId   = serverId;
  _setupLastLogLen = 0;

  // Сброс UI
  document.getElementById('setup-modal-title').textContent = `Настройка: ${serverName}`;
  document.getElementById('setup-modal-subtitle').textContent = 'Пожалуйста, не закрывайте окно';
  document.getElementById('setup-modal-icon').className = 'fas fa-cog fa-spin text-brand-400';
  document.getElementById('setup-error-block').classList.add('hidden');
  document.getElementById('setup-btn-retry').classList.add('hidden');
  document.getElementById('setup-btn-cancel').classList.remove('hidden');
  document.getElementById('setup-btn-done').classList.add('hidden');

  for (let i = 1; i <= 5; i++) {
    document.getElementById(`setup-step-${i}-icon`).textContent = '⏸️';
    const log = document.getElementById(`setup-step-${i}-log`);
    log.innerHTML = '';
    log.classList.add('hidden');
    document.getElementById(`setup-step-${i}-chevron`).classList.remove('rotate-180');
  }
...
function _startSetupPolling() {
  ...
  _pollSetupStatus(); // сразу первый запрос
}
...
async function _pollSetupStatus() {
  ...
  try {
    ...
    if (data.setup_status === 'done' || data.setup_status === 'failed') {
      clearInterval(_setupPollTimer);
      _onSetupFinished(data);
    }
  } catch (e) {
...
function _renderSetupProgress(data) {
  ...
  // Распределяем строки лога по шагам
  const stepLogs = { 1: [], 2: [], 3: [], 4: [], 5: [] };
  let curStep = 0;
  for (const line of lines) {
    const m = line.match(/^\[(\d)/);
    if (m) curStep = parseInt(m[1]);
    if (curStep >= 1 && curStep <= 5) stepLogs[curStep].push(line);
  }

  for (let i = 1; i <= 5; i++) {
    const iconEl = document.getElementById(`setup-step-${i}-icon`);
    const logEl  = document.getElementById(`setup-step-${i}-log`);

    // Иконка статуса
    if (i < currentStep) {
      // Шаг завершён — смотрим есть ли ❌ в логе
      const hasError = stepLogs[i].some(l => l.includes('❌'));
      iconEl.textContent = hasError ? '❌' : '✅';
    ...
    if (stepLogs[i].length > 0) {
      logEl.innerHTML = stepLogs[i]
        .map(l => `<div class="${_logLineClass(l)}">${_escHtml(l)}</div>`)
        .join('');
      // Автораскрываем текущий шаг
      if (i === currentStep) {
        logEl.classList.remove('hidden');
        document.getElementById(`setup-step-${i}-chevron`).classList.add('rotate-180');
      }
    }
  }
}
...
function _onSetupFinished(data) {
  ...
  document.getElementById('setup-modal-icon').className = success
  ...
  for (let i = 1; i <= 5; i++) {
    const iconEl = document.getElementById(`setup-step-${i}-icon`);
    if (iconEl.textContent === '⏸️') continue; // не дошли до шага
  ...
  if (success) {
    document.getElementById('setup-btn-cancel').classList.add('hidden');
    document.getElementById('setup-btn-done').classList.remove('hidden');
    loadServers();
  } else {
    if (data.setup_error) {
    ...
    document.getElementById('setup-btn-retry').classList.remove('hidden');
  }
}

function toggleSetupStep(num) {
  const logEl     = document.getElementById(`setup-step-${num}-log`);
  const chevron   = document.getElementById(`setup-step-${num}-chevron`);
  const isHidden  = logEl.classList.contains('hidden');
  if (isHidden) {
    logEl.classList.remove('hidden');
...
async function retryServerSetup() {
  ...
  for (let i = 1; i <= 5; i++) {
    document.getElementById(`setup-step-${i}-icon`).textContent = '⏸️';
    document.getElementById(`setup-step-${i}-log`).innerHTML = '';
    document.getElementById(`setup-step-${i}-log`).classList.add('hidden');
  }
