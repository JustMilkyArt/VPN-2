...
class ServerRole(str, enum.Enum):
    RU = "RU"       # Entry node / routing brain
    EU = "EU"       # Exit node
    MIXED = "MIXED" # Both roles
...
class Server(Base):
    ...
    country = Column(String(10), nullable=False, default="??")  # ISO code: RU, DE, NL, etc.
    ...
    ssh_key = Column(Text, nullable=True)          # Private key (PEM)
    ssh_password = Column(String(255), nullable=True)  # Optional password
    ...
    # Domain for NaiveProxy/Trojan TLS
