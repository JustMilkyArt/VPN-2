...
def gen_xray_vless_reality_inbound(
    port: int,
    uuid_str: str,
    public_key: str,
    private_key: str,
    ...
    - Port: 443 (caller must pass 443)
    - Flow: xtls-rprx-vision
    - serverName: matches location (caller passes appropriate SNI)
    """
    return {
        "tag": f"vless-in-{port}",
        "listen": "0.0.0.0",
...
def build_naiveproxy_caddy_config(domain: str, password: str, port: int = 443) -> str:
    """Generate Caddyfile for NaiveProxy via Caddy with TLS.

    Per spec:
    - Requires a valid domain with DNS pointing to this server
    - TLS is provisioned automatically via Let's Encrypt (ACME)
    - Listens on port 443
    - domain MUST be a real FQDN — do NOT call without valid domain
    """
    return f"""{domain}:{port} {{
  tls {{
    # Caddy auto-provisions TLS via Let's Encrypt for this domain
  }}
  route {{
    forward_proxy {{
...
# Mapping country code → best SNI reachable from that location
REALITY_SNI_BY_COUNTRY = {
    "RU": "www.microsoft.com",
    "CN": "www.apple.com",
    "IR": "www.amazon.com",
    "BY": "www.microsoft.com",
    "KZ": "www.microsoft.com",
    "UA": "www.cloudflare.com",
    "TR": "www.cloudflare.com",
    "IN": "www.amazon.com",
}


def get_reality_server_name(country: str = None) -> str:
    """Return best SNI domain for Reality based on server country/location."""
    if country and country.upper() in REALITY_SNI_BY_COUNTRY:
        return REALITY_SNI_BY_COUNTRY[country.upper()]
    return REALITY_SNI_DEFAULT
...
def gen_awg_server_config(
    ...
    - Jmin=40, Jmax=70 (junk packet size bytes)
    - S1/S2: random values 15-150 (generated once per server, stored in DB)
    - MTU=1420
    - PersistentKeepalive=25 (set on client side)
    """
    clients = clients or []
    # S1/S2: random in range 15-150 per spec; caller should store and reuse same values
    if s1 is None:
...
def gen_awg_client_config(
    ...
    IMPORTANT: S1/S2 MUST match the server config exactly.
    Pass the same s1/s2 values that were used for gen_awg_server_config.
    MTU=1420, PersistentKeepalive=25 per spec.
    """
    # If caller doesn't pass s1/s2, generate new ones (won't match server — only for testing)
    if s1 is None:
        s1 = random.randint(15, 150)
    if s2 is None:
        s2 = random.randint(15, 150)
...
# AmneziaWG obfuscation (must match server exactly)
Jc = {junk_packet_count}
Jmin = {junk_packet_min_size}
Jmax = {junk_packet_max_size}
S1 = {s1}
